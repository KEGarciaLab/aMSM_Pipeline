...
def generate_avg_maps(pre_msm_data: str, msm_data: str, subject: str, younger_timepoint: str, older_timepoint: str, max_cp: str | None=None, max_anat: str | None=None, younger_uses_mcribs: bool=False, older_uses_mcribs: bool=False):
    ...
    if younger_uses_mcribs:
        print(f"{datetime.now()}[INFO] Younger timepoint uses MCRIBS pipeline")
        print(f"{datetime.now()}[FUNCTION] Calling get_files_mcribs(dataset=pre_msm_data, subject=subject, time_point=younger_timepoint, is_rescaled=True)")
        younger_files = get_files_mcribs(dataset=pre_msm_data, subject=subject, time_point=younger_timepoint, is_rescaled=True)
        print()
    else:
        print(f"{datetime.now()}[INFO] Younger timepoint uses standard pipeline")
        print(f"{datetime.now()}[FUNCTION] Calling get_files(dataset=pre_msm_data, subject=subject, time_point=younger_timepoint)")
        younger_files = get_files(dataset=pre_msm_data, subject=subject, time_point=younger_timepoint)
        print()
    
    if older_uses_mcribs:
        print(f"{datetime.now()}[INFO] Older timepoint uses MCRIBS pipeline")
        print(f"{datetime.now()}[FUNCTION] Calling get_files_mcribs(dataset=pre_msm_data, subject=subject, time_point=older_timepoint, is_rescaled=True)")
        older_files = get_files_mcribs(dataset=pre_msm_data, subject=subject, time_point=older_timepoint, is_rescaled=True)
        print()
    else:
        print(f"{datetime.now()}[INFO] Older timepoint uses standard pipeline")
        print(f"{datetime.now()}[FUNCTION] Calling get_files(dataset=pre_msm_data, subject=subject, time_point=older_timepoint)")
        older_files = get_files(dataset=pre_msm_data, subject=subject, time_point=older_timepoint)
        print()
...
def get_files_mcribs(dataset: str, subject: str, time_point: str, is_rescaled=False):
    ...
    left_anatomical_surface = find(patterns=["lh.midthickness_svd_rot.surf.gii",f"lh.{subject}{time_point}_midthickness_711-2N.surf.gii", f"lh.{subject}{time_point}_midthickness_711-2B.surf.gii", "lh.midthickness.surf.gii"], search_path=subject_dir)
    print()
    
    print(f'{datetime.now()}[FUNCTION] find(patterns="rh.midthickness.surf.gii", search_path=subject_dir)')
    right_anatomical_surface = find(patterns=["rh.midthickness_svd_rot.surf.gii",f"rh.{subject}{time_point}_midthickness_711-2N.surf.gii", f"rh.{subject}{time_point}_midthickness_711-2B.surf.gii", "rh.midthickness.surf.gii"], search_path=subject_dir)
    print()
